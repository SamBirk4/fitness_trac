# Comand line interface CLI application that keeps a record of animals I own
   ...
   # Input verification
    while True:
        try:
            ...
            # Input verification
            if not typeAnimal == 'c' and not typeAnimal == 'd' and typeAnimal !='a':
                raise ValueError('Invalid input for animal type')
            else:
                break
        except ValueError:
            print('Oh no. Please enter a valid type for animal.')
            pass
    while True:
        try:
            print("\n\nEnter animal name:")
            name = input(">>>")
            if not re.search(',', name) == None:
                raise ValueError
            else:
                break
        except ValueError:
            print('Please do not use "," in your name')
    ...
    while True:
        try:
            ...
            if option == 'q':
                ...
                raise ValueError('Invalid menue option')
        except ValueError:
